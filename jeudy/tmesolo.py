...
class PasseStrategy(BaseStrategy):
    ...
    def compute_strategy(self, state,id_team, id_player):
        ...
        if a.shoot_tme() == 0:     #Passe
            if a.enemy_close_position().distance(a.my_position) <= 30*settings.PLAYER_RADIUS:
                sa=SoccerAction(Vector2D(a.ball_position-a.my_position),a.position_j1()-a.ball_position)  
               
                sa.shoot.norm=-sa.shoot.norm
                return sa
            else:
                return SoccerAction()
        

        if a.ball_position.distance(a.my_position) >= 4*settings.PLAYER_RADIUS: # On reduit vitesse
            print "proche"
            sa=SoccerAction(a.ball_position-a.my_position,Vector2D())
            sa.acceleration.norm=1
            return sa
        
        if a.ball_position.distance(a.my_position) >= settings.PLAYER_RADIUS:
            print "tres proche"
            sa=SoccerAction(a.ball_position-a.my_position,Vector2D())
            sa.acceleration.norm=0.1
            return sa
